...
//   - "FIRST:" / "Your job:" / "THE TEST:" 开头的系统指令注入
...
const TUI_SYSTEM_PREFIX_RE = /^(FIRST|Your job|THE TEST)[\s:].*/i
...
export function cleanUserText(raw: string): string | null {
  ...
  text = text.trim()
  if (!text) return null
  // TUI 注入的系统指令（FIRST:/Your job/THE TEST: 开头），整条丢弃
  if (TUI_SYSTEM_PREFIX_RE.test(text)) return null
  return text
}
