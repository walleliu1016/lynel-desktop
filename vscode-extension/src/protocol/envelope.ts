...
export interface SessionEnvelope {
  id: string; time: number; role: 'user' | 'agent';
  sessionId?: string; turn?: string; subagent?: string;
  claudeUuid?: string; claudeMsgId?: string; usage?: SessionUsage;
  ev: SessionEvent;
}
...
export type CreateEnvelopeOptions = {
  id?: string; time?: number; sessionId?: string; turn?: string;
  subagent?: string; claudeUuid?: string; claudeMsgId?: string;
  usage?: SessionUsage; seq: number; agent?: string;
};

export function createEnvelope(role: 'user' | 'agent', ev: SessionEvent, opts: CreateEnvelopeOptions): LynelEnvelope {
  const env: LynelEnvelope = { id: opts.id ?? randomUUID(), time: opts.time ?? Date.now(), role, seq: opts.seq, ev };
