...
interface SelectedItem { question_key: string; option_ids?: { option_id?: string[] }; }

interface TemplateCardEventPayload {
  event_key?: string; task_id?: string; card_type?: string;
  selected_items?: { selected_item?: SelectedItem[] };
}

export interface TemplateCardEventFrame {
  body: {
    chatid?: string; from?: { userid?: string };
    event: { eventtype: string; template_card_event: TemplateCardEventPayload };
  };
}
...
export class WeComCardEventHandler {
  ...
  async handle(frame: TemplateCardEventFrame): Promise<void> {
    try {
      ...
      if (!chatId) { logger.warn('[wecom-card-event-handler] 无法获取 chatId'); return; }
      ...
      if (!parsed) { await this.sendReply(chatId, HINT_UNRECOGNISED_KEY); return; }

      const { action, requestId } = parsed;
      const state = this.store.get(requestId);
      if (!state || state.status !== 'pending') {
        await this.sendReply(chatId, HINT_ALREADY_HANDLED, requestId); return;
      }

      if (action === 'allow' || action === 'deny') {
        const ok = permissionBroker.resolve(requestId, action, 'wecom');
        if (ok) { this.store.resolve(requestId, action); await this.updateOrNotify(requestId, chatId, action, frame); }
        else { await this.sendReply(chatId, HINT_ALREADY_HANDLED, requestId); }
        return;
      }

      if (action === 'submit' || action === 'answer') {
        const pending = permissionBroker.listPending().find((p) => p.id === requestId);
        if (!pending) { await this.sendReply(chatId, HINT_ALREADY_HANDLED, requestId); return; }

        const answers = this.buildAnswers(requestId, pending.request, tce?.selected_items);
        if (answers instanceof Error) { await this.sendReply(chatId, answers.message, requestId); return; }
        ...
        if (ok) { this.store.resolve(requestId, 'allow', answers); await this.updateOrNotify(requestId, chatId, 'allow', frame); }
        else { await this.sendReply(chatId, HINT_ALREADY_HANDLED, requestId); }
        return;
      }
  ...
  private parseEventKey(eventKey: string | undefined): { action: string; requestId: string } | undefined {
    ...
    const action = parts[1]; const requestId = parts[2];
    if (!action || !requestId) return undefined;
    return { action, requestId };
  }

  private buildAnswers(
    requestId: string, req: PermissionRequest,
    ...
    if (selectedItems.length === 0) return new Error(HINT_NO_SELECTION);
    ...
    if (Object.keys(answers).length === 0) return new Error(HINT_NO_SELECTION);
    return answers;
  }

  private extractIndex(key: string, segmentIndex: number, defaultValue: number): number {
    const parts = key.split(':');
    if (parts.length <= segmentIndex) return defaultValue;
    const parsed = parseInt(parts[segmentIndex], 10);
  ...
  private async updateSubmittedCard(requestId: string, qIdx: number, frame: TemplateCardEventFrame): Promise<void> {
    const state = this.store.get(requestId);
        ...
        body: {
          ...
          event: {
            ...
            template_card_event: {
              ...
              task_id: requestId, response_code: msgid,
            },
          },
        },
      };
      await this.updateCard(updateFrame, {
        card_type: 'text_notice', source: { desc: 'Lynel', desc_color: 0 },
  ...
  private async updateOrNotify(
    requestId: string, chatId: string, decision: 'allow' | 'deny', frame: TemplateCardEventFrame,
  ): Promise<void> {
    const state = this.store.get(requestId);
    if (!state) return;
      ...
      const updateFrame = {
        ...
        body: {
          ...
          event: {
            ...
            template_card_event: {
              ...frame.body.event.template_card_event,
              task_id: requestId, response_code: state.msgid,
            },
          },
        },
      };
    ...
    } catch { await this.sendReply(chatId, text, requestId); }
  }
}
