...
- **飞书通道** — 占位支持，后续实现
- **三段式布局（Trace 面板）** — 左侧 SessionList（280px） | 中间终端 + GlobalTabs（flex:1） | 右侧 TraceSidebar（200px）；缩略图实时展示 API 请求列表（状态/seq/model/延迟/费用），点击缩略行展开 TraceOverlay 覆盖层（`clamp(360px, 35%, 45%)` 自适应宽度）查看完整请求详情与 diff；数据源自本地 API 网关落盘的 `raw/*.json`，通过 `watch(activeSessionId)` 统一自动加载
...
│       ├── router/              # hash 路由 + 鉴权守卫
│       ├── stores/              # Pinia：auth / sessions / settings / hooks / trace
...
│       │                        #   trace/{TraceSidebar,TraceOverlay,RequestDetailPane} /
