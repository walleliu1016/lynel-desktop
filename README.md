...
- **xterm.js 原生终端** — 中间区域嵌入 xterm.js，PTY 驱动交互式 agent
- **三段式布局** — 左侧会话列表 | 中间终端 | 右侧 Trace 面板，均可收起展开
- **API 网关代理** — 本地拦截 API 流量，按 agent 适配 Anthropic / OpenAI Responses / Chat 格式，提取 prompt / tool_use / tool_result 等阶段数据
...
- **权限拦截** — 经 Claude hooks 与各 agent 权限策略，统一走权限仲裁器审批
...
启动应用后设置本地账户密码，登录后自动加载所有历史会话。选择 Project 目录新建会话（可选 Claude Code / Codex / OpenCode / OMP）即可开始使用。
