...
- **xterm.js 原生终端渲染** — 中间区域嵌入 xterm.js，Go 端用 PTY 运行交互式 `claude`，原始 ANSI 字节直接渲染
...
- **AdoptSession / OpenSessionTerminal** — 历史 session 先注册到 App；点击打开时按需 `claude --resume <sid>` 进入交互式 PTY，已打开的 session 切回时复用 xterm 页面
