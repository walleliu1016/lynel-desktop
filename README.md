...
- **五 tab 设置页** — General（基础设置） / Hooks（Form/JSON 双视图编辑器） / Cloud（占位） / Provider（模型供应商） / Channel（企业微信）
- **企业微信通道（双向）** — 通过 `@wecom/wecom-openclaw-plugin` 连接企业微信 Bot；Claude 的 prompt / AI 回复 / 工具 / 错误等事件自动推送到企业微信，企业微信收到的文字/图文混排消息转发到当前活动会话
...
3. 事件格式化为企业微信 Markdown 后，调用 `@wecom/wecom-openclaw-plugin` 的 `outbound.sendText`。
4. 插件优先使用 Bot WebSocket 发送；连接不可用时自动回退到 Agent HTTP API（需配置 `agent.corpId` / `corpSecret` / `agentId`）。
5. `send()` 改为 fire-and-forget，不阻塞 dispatcher 与主进程事件循环；首次事件触发前已在 `updateConfig()` 里预连 WebSocket。
