...
| 持久化 | 本地 JSON + bcrypt | v1 不上 DB；`~/.lynel-desktop/` 五个文件搞定 |
...
│      ├── settings      ~/.lynel-desktop/settings.json                                             │
│      ├── instance      ~/.lynel-desktop/instance.json：会话状态持久化（30s 防抖）                  │
...
│   ├── settings/                # ~/.lynel-desktop/settings.json 读写
│   ├── instance/                # ~/.lynel-desktop/instance.json 持久化（30s 防抖写盘）
│   ├── single/                  # flock (Unix) / LockFileEx (Windows) 单例锁
...
```bash
# 1) 启动 GUI
open ./build/bin/lynel-desktop.app
# 或直接：./build/bin/lynel-desktop.app/Contents/MacOS/lynel-desktop

# 2) 首次登录：用户名 + 密码（bcrypt hash 写到 ~/.lynel-desktop/auth.json）

# 3) 重置密码：删 auth.json，下次启动会重新引导
rm ~/.lynel-desktop/auth.json
```
...
| 文件 | 路径 | 权限 |
|---|---|---|
| Ease 配置 | `~/.lynel-desktop/settings.json` | 读写 |
| 鉴权（bcrypt hash） | `~/.lynel-desktop/auth.json` | 读写 |
| 会话状态（持久化） | `~/.lynel-desktop/instance.json` | 读写（30s 防抖） |
| SessionStart hook 脚本 | `~/.lynel-desktop/session-start.sh`（macOS/Linux）<br>`~/.lynel-desktop/session-start.ps1`（Windows） | 写入（755 / 644） |
| API 网关阶段数据 | `~/.lynel-desktop/projects/<encoded-project>/<sid>-calls.jsonl` | 写入（追加） |
| 单例锁 | `~/Library/Application Support/lynel-desktop/singleton.lock`（macOS）<br>`$XDG_RUNTIME_DIR/lynel-desktop/singleton.lock` 或 `~/.config/lynel-desktop/singleton.lock`（Linux）<br>`%AppData%\lynel-desktop\singleton.lock`（Windows） | 内核自动管理 |
| 日志（lumberjack 滚动） | `~/.lynel-desktop/logs/*.log` | 读写 |
...
Wails 在 macOS 上把 `wailsruntime.LogInfo` 写到 `~/Library/Logs/<bundle-id>/`，可以用 `Console.app` 或 `log stream --predicate 'subsystem == "com.akke.lynel-desktop"'` 看。
...
- 🐛 **生产 build 偶现黑屏** —— dev 模式 OK，prod build 启动后 WebView 偶现不渲染 UI。原因仍在排查（见 `docs/superpowers/plans/2026-06-27-lynel-desktop.md` Task 44 后续）。workaround：用 `wails dev` 开发，或在 prod 加 `-devtools` flag 复现。
