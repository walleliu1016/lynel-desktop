...
> ⚠️ **本文档已过时**（描述的是早期三栏布局、本地账户登录、Hooks/通道设置页，均已变更）。请以 **[user-guide.md](user-guide.md)** 为准。
...
| `/screenshot` | 截取当前会话终端画面（最后 50 行），以图片发送到企业微信 |
...
在 **设置 → Cloud** 中配置云服务地址和 Token 后：

- **阶段事件**：自动批量推送（3 秒合并窗口，最多 50 条）至云端 `/api/envelope/push`
- **会话信息**：新建/打开/接管会话时自动同步元数据至 `/api/sessions/sync`
- **健康检查**：Cloud 设置页提供测试连接按钮

### 终端截图

- 在企业微信中发送 `/screenshot` 即可截取当前绑定会话的终端画面
- 截图为 PNG 图片，包含终端最后 50 行纯文本内容（清除 ANSI 控制码后渲染）
- 需要 `@napi-rs/canvas` 支持（已内置）
