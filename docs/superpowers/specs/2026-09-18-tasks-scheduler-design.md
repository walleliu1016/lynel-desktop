...
**残留（已接受）：恢复只改行，不杀进程。** App 被**硬杀**（`taskkill /F`、SIGKILL、断电、系统崩溃）时，
正常退出路径的 `killAllRuns()`（tree-kill）根本没机会执行，而 run 是 `detached: true` 起的 ——
那个 claude 进程会脱离父进程继续跑，直到自己跑完或被系统回收；它的输出管道已经断开，谁也不知道它在干嘛。
下一次启动时 `recoverStaleRuns()` 只把它的 run 标成 `interrupted`，**不会**去杀它，于是它可能正与
随后的 `--resume` 并发写同一个 jsonl（D11 想排除的正是这种并发写）。
**不为它做 pid 持久化 + 启动扫杀**：跨平台「判活 + 杀树」不可靠（pid 会被系统复用，误杀代价更大），
而收益只是一个非常规终止路径下的小概率损坏 —— 记为接受债务。
